    ...
    Sub UpdateControls()
        ...
        If offset = 0 Then
            ...
            If qualityParam.Value > 0 Then
                lv.Items.Add(New ListViewItem({Localization.Translate("Quality"), Localization.Translate(qualityParam.OptionText)}))
            Else
                Dim crfParam = If(Params.ConstantRateFactorLow.Visible, Params.ConstantRateFactorLow, Params.ConstantRateFactorHigh)
                Dim quantParam = If(Params.QuantizationParameterLow.Visible, Params.QuantizationParameterLow, Params.QuantizationParameterHigh)
                Dim paramValue = If(crfParam.Visible, crfParam.Value, quantParam.Value)

                lv.Items.Add(New ListViewItem({Localization.Translate("Quality"), GetQualityCaption(paramValue)}))
            End If
        End If
        Dim speedParam = If(Params.SpeedHigh.Visible, Params.SpeedHigh, Params.SpeedLow)
        If speedParam.Value > 0 Then
            lv.Items.Add(New ListViewItem({Localization.Translate("Speed"), Localization.Translate(speedParam.OptionText)}))
        Else
            Dim presetParam = Params.Preset
            lv.Items.Add(New ListViewItem({Localization.Translate("Preset"), Localization.Translate(presetParam.OptionText)}))
        End If
